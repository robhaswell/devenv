...
function! GetPhpIndent()
    ...
    " ROB WAS HERE: commented out below lines so that PHP delims stay on same
    " line
"    if cline =~# '^\s*<?' && cline !~ '?>'
"	return 0
"    endif
...
"    endif
	...
	" let ind = ind - &sw
	" ROB WAS HERE: Commented out above line because I like my switch/case
	" like this:
	" switch ($foo) {
	"     case 'bar':
	"         $baz = 'boo';
    ...
    if last_line =~# s:notPhpHereDoc
	let ind = ind - &sw
    endif
    " ROB WAS HERE: Last three lines added to take me back a line after break
